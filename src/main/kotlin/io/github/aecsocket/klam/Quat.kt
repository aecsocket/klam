...
data class Quat(@JvmField var x: Float, @JvmField var y: Float, @JvmField var z: Float, @JvmField var w: Float) {
    ...
    inline operator fun plus(s: Float)  = Quat(x + s, y + s, z + s, w + s)
    inline operator fun minus(s: Float) = Quat(x - s, y - s, z - s, w - s)
    inline operator fun times(s: Float) = Quat(x * s, y * s, z * s, w * s)
    inline operator fun div(s: Float)   = Quat(x / s, y / s, z / s, w / s)
    ...
    inline operator fun timesAssign(q: Quat) {
    ...
    inline operator fun times(q: Quat) = Quat(this).apply { this *= q }
    ...
    inline fun map(block: (Float) -> Float) = Quat(block(x), block(y), block(z), block(w))
