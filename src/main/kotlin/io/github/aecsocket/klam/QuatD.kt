...
data class DQuat(@JvmField var x: Double, @JvmField var y: Double, @JvmField var z: Double, @JvmField var w: Double) {
    companion object {
        val Identity get() = DQuat(0.0, 0.0, 0.0, 1.0)
    ...
    inline fun map(block: (Double) -> Double) = DQuat(block(x), block(y), block(z), block(w))

    inline operator fun plus(s: Double)  = DQuat(x + s, y + s, z + s, w + s)
    inline operator fun minus(s: Double) = DQuat(x - s, y - s, z - s, w - s)
    inline operator fun times(s: Double) = DQuat(x * s, y * s, z * s, w * s)
    inline operator fun div(s: Double)   = DQuat(x / s, y / s, z / s, w / s)
    ...
    inline operator fun times(q: DQuat) = DQuat(this).apply { this *= q }

    inline operator fun timesAssign(q: DQuat) {
        from(
...
inline operator fun Double.times(q: DQuat) = DQuat(this * q.x, this * q.y, this * q.z, this * q.w)
